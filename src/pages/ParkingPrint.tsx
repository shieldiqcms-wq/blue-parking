...
import { listSessions } from '@/services/parking'
 ...
 *   1) ملخص الفترة
 *   2) جدول الموقف يوماً بيوم (الزيارات + الاشتراكات)
 *   3) لكل يوم: السيارات التي دخلت فيه، والمشتركة معلَّمة بقيمة اشتراكها،
 *      ثم دفعات الاشتراكات المقبوضة في ذلك اليوم.
 *
 * نستخدم طباعة المتصفح (حفظ كـ PDF) بدل مكتبة PDF: المتصفح يرسم العربية
 * ويوصل الحروف بشكل صحيح، والنص يبقى قابلاً للبحث والنسخ، ويقسّم الصفحات
 * تلقائياً مع تكرار رؤوس الجداول.
 */
export function ParkingPrintPage() {
  ...
  const data = useAsync(async () => {
    const [report, sessions, subPayments, settings] = await Promise.all([
      getReport(from, to),
        ...
        {data.data && totals && (
          <article className="print-sheet flex flex-col gap-5 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-sand-200 sm:p-6 print:rounded-none print:p-0 print:shadow-none print:ring-0">
            ...
            <p className="-mt-3 text-[11px] text-slate-500">
              ...
              الاشتراكات داخلة في دخل يوم قبضها وشهره.
            </p>
            ...
            <section>
              ...
              <div className="overflow-x-auto print:overflow-visible">
                <table className="print-table w-full min-w-[40rem] text-sm print:min-w-0">
                  <thead>
                    <tr>
                      <th>التاريخ</th>
                      <th>اليوم</th>
                    ...
                    {days.map((d: ReportDay) => {
                      ...
                      return (
                        <tr key={d.day} className={empty ? 'text-slate-400' : undefined}>
                          <td><span className="num">{formatDateNumeric(d.day)}</span></td>
                          <td>{weekday(d.day)}</td>
                          ...
                          <td className="font-semibold"><span className="num">
                            {formatMoney(d.parking_revenue)}
                          </span></td>
...
function DaySection({
  ...
  const collected = sessions.reduce(
    (sum, s) => sum + Number(s.amount_collected ?? 0),
    0,
  )
  ...
  return (
    <div className="print-day flex flex-col gap-2">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 rounded-lg bg-brand-50 px-3 py-2 print:bg-brand-50">
      ...
      {sessions.length > 0 && (
        <div className="overflow-x-auto print:overflow-visible">
          <table className="print-table w-full min-w-[40rem] text-xs print:min-w-0">
            <thead>
              <tr>
                <th className="w-8">#</th>
                <th>اللوحة</th>
              ...
              {sessions.map((s, index) => (
                <tr key={s.id}>
                  ...
                  <td className="font-semibold"><span className="num">
                    {s.session_type === 'monthly'
                      ? '—'
                      : s.amount_collected !== null
                        ? formatMoney(s.amount_collected)
                        : '—'}
                  </span></td>
                  <td className="min-w-[9rem] whitespace-normal text-[11px]">
      ...
      {payments.length > 0 && (
        <div className="overflow-x-auto print:overflow-visible">
          <p className="mb-1 text-xs font-bold text-violet-800">
            دفعات اشتراكات مقبوضة في هذا اليوم
          </p>
          <table className="print-table w-full min-w-[34rem] text-xs print:min-w-0">
            <thead>
              <tr>
                <th>اللوحة</th>
                <th>المالك</th>
...
function SummaryBox({
  ...
  label: string
  value: string
  tone?: 'slate' | 'blue' | 'green' | 'red' | 'violet'
}) {
  const color = {
